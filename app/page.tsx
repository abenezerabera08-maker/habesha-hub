...
function formatPrice(tiers: { price: number }[]): string {
  ...
  return min === 0 ? "Free" : `From ETB ${min.toFixed(2)}`;
}
...
export default function DiscoverPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data, error } = await supabase
        .from("events")
        .select(
          `id, title, event_date, location, city_id, ticket_tiers ( price )`,
        )
        .eq("status", "published")
      ...
      const scored = (data as Event[]).map((event) => {
        ...
        if (eventInterests.some((id) => viewerInterestIds.includes(id)))
          score += 1;
        return { event, score };
      });
  ...
  if (loading)
    return (
      <p style={{ maxWidth: 720, margin: "40px auto", padding: "0 16px" }}>
        Loading...
      </p>
    );
  ...
  return (
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "40px 16px" }}>
      ...
      />
      <p style={{ marginBottom: 32, color: "#555", textAlign: "center" }}>
