...
export default async function AccountPage() {
  ...
  const [{ data: profileRow }, ordersRes, interestsRes] = await Promise.all([
    supabase
      .from('profiles')
  ...
  const eventsAttended = ordersRes.count ?? 0
  const interestCount = interestsRes.count ?? 0
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '24px 16px 96px' }}>
      ...
      {/* 4. Host an Event + Manage Your Events */}
      <HostEventSection />
